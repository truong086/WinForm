...
{
    public partial class nganhdulich : Form
    {
        DataConnection daa;
        SqlDataAdapter da;
        string mataikhoantt = "", tentaikhoan = "", matkhau = "", capquyen = "";
        sinhvienBLL bll;
        public nganhdulich(string mataikhoantt, string tentaikhoan, string matkhau, string capquyen)
        {
            InitializeComponent();
            bll = new sinhvienBLL();
        ...
        public void getAlldulich()
        {
            DataTable dt = bll.getAllKhoadulich();
            nganhdulichs.DataSource = dt;
        }
        private void nganhdulich_Load(object sender, EventArgs e)
        {
        ...
        int id;
        private void nganhdulichs_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            int index = e.RowIndex;
            if(index >= 0)
            {
                id = Int32.Parse(nganhdulichs.Rows[index].Cells["manganhtt"].Value.ToString());
                manganhs.Text = nganhdulichs.Rows[index].Cells["manganh"].Value.ToString();
                makhoas.Text = nganhdulichs.Rows[index].Cells["makhoa"].Value.ToString();
                tennganhs.Text = nganhdulichs.Rows[index].Cells["tennganh"].Value.ToString();
            }
        }
        ...
        private void edit_Click(object sender, EventArgs e)
        {
            if (checkData())
            {
                ...
                if (bll.EditNganh(sv))
                {
                    if(sv.makhoaa == 114)
                    {
                        bll.EditNganhdulich(sv);
                    }
                    if(sv.makhoaa != 114)
                    {
                        if(MessageBox.Show("Bạn đang sửa lại thông tin không khớp với ngành du lịch, nếu tiếp tục sửa thì bản ghi này sẽ bị xóa khởi bảng ngành du lịch", "Cảnh báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                        {
                            bll.EditNganh(sv);
                            if (sv.makhoaa == 113)
                            {
        ...
        private void delete_Click(object sender, EventArgs e)
        {
            if(MessageBox.Show("Bạn có chắc chắn muốn xóa bản ghi này không?", "Cảnh báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                tbsinhvien cv = new tbsinhvien();
                cv.manganhh = int.Parse(manganhs.Text);
                if (bll.DeleteNganh(cv) && bll.DeleteNganhdulich(cv))
                    getAlldulich();
                else
        ...
        private void timkiems_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                DataTable dt = bll.TimkiemNganhdulich(value);
                nganhdulichs.DataSource = dt;
            }
            else
            {
