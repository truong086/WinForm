...
{
    public partial class sinhvienk16 : Form
    {
        DataConnection dcs;
        SqlDataAdapter da;
        string mataikhoantt = "", tentaikhoan = "", matkhau = "", capquyen = "";
        sinhvienBLL bll;
        public sinhvienk16(string mataikhoantt, string tentaikhoan, string matkhau, string capquyen)
        {
            InitializeComponent();
            bll = new sinhvienBLL();
        ...
        public void getAllk16()
        {
            DataTable dt = bll.getAllsinhvienk16();
            svk16.DataSource = dt;
        }
        
        private void sinhvienk16_Load(object sender, EventArgs e)
        {
            if (capquyen == "1")
            {
                quyen.Text = "Admin";
        ...
        int id;
        private void svk16_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            int index = e.RowIndex;
            if(index >= 0)
            {
                id = Int32.Parse(svk16.Rows[index].Cells["masvtt"].Value.ToString());
                masvs.Text = svk16.Rows[index].Cells["msv"].Value.ToString(); // Lấy ra từng trường dữ liệu, và chuyển về kiểu "ToString()", lấy ra trường dữ liệu "masv"
                madoituongs.Text = svk16.Rows[index].Cells["madoituong"].Value.ToString(); // Lấy ra từng trường dữ liệu, và chuyển về kiểu "ToString()", lấy ra trường dữ liệu "madoituong"
                malops.Text = svk16.Rows[index].Cells["malop"].Value.ToString(); // Lấy ra từng trường dữ liệu, và chuyển về kiểu "ToString()", lấy ra trường dữ liệu "malop"
                hotens.Text = svk16.Rows[index].Cells["hoten"].Value.ToString(); // Lấy ra từng trường dữ liệu, và chuyển về kiểu "ToString()", lấy ra trường dữ liệu "hoten"
                ngaysinhs.Text = svk16.Rows[index].Cells["ngaysinh"].Value.ToString(); // Lấy ra từng trường dữ liệu, và chuyển về kiểu "ToString()", lấy ra trường dữ liệu "ngaysinh"
                diachis.Text = svk16.Rows[index].Cells["diachi"].Value.ToString(); // Lấy ra từng trường dữ liệu, và chuyển về kiểu "ToString()", lấy ra trường dữ liệu "diachi"
                gioitinhs.Text = svk16.Rows[index].Cells["gioitinh"].Value.ToString();
            }
        }
        ...
        private void edit_Click(object sender, EventArgs e)
        {
            if (checkData())
            {
                ...
                //sv.masvtt = id; // Nếu update lại dữ liệu vào bảng sinh viên(sinhvien) bị lỗi thì xóa dòng này đi
                ...
                {
                    if (MessageBox.Show("Bản ghi bạn vừa nhập không trùng với tên lớp của khóa 16, nếu tiếp tục sửa bản ghi này sẽ bị xóa", "Cảnh báo", MessageBoxButtons.YesNo, MessageBoxIcon.Error) == DialogResult.Yes)
                    {
                        bll.UpdateSinhVien(sv);
                        ...
                        if (sv.malop == 5 || sv.malop == 6 || sv.malop == 8 || sv.malop == 15)
                        {
        ...
        {
            if(MessageBox.Show("Bạn có chắc chắn muốn xóa bản ghi này không", "Cảnh báo", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)
            {
                tbsinhvien sv = new tbsinhvien();
                sv.masv = int.Parse(masvs.Text);
                if (bll.DeleteSinhVienk16(sv) && bll.DeleteSinhVien(sv))
                {
                    tbsinhvien cv = new tbsinhvien();
                    cv.masv = int.Parse(masvs.Text);
                    cv.madoituong = int.Parse(madoituongs.Text);
        ...
        private void find_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                DataTable dt = bll.FindSinhVienk16(value);
                svk16.DataSource = dt;
            }
            else
            {
